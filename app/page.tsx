'use client'

import Link from 'next/link'
import { useMemo, useState } from 'react'
import { supabase } from '@/lib/supabase'

type PostRow = {
  id: string
  board: string
  title: string
  content: string
  author_anon_id: string
  created_at: string
}

type CommentRow = {
  id: string
  post_id: string
  content: string
  author_anon_id: string
  created_at: string
}

function getAnonId() {
  const key = 'chasjabara_anon_id'
  let v = localStorage.getItem(key)
  if (!v) {
    v = `anon_${crypto.randomUUID().slice(0, 8)}`
    localStorage.setItem(key, v)
  }
  return v
}

export default function Home() {
  const intro = `이 사이트는 ChatGPT AI 기준으로 만들었으며 그 유저의 정보가 많이 없을 경우 자세하게 나오지 않을 수 있습니다.
정보가 쌓일수록 더 정확하게 그 사람의 스타일을 알려줍니다.
익명의 게시판이며 자유롭게 이용하셨으면 합니다.

테일즈런너 차사잡아라 유저분들 모두모두 행복한 한 판을 즐겼으면 합니다.`

  const boards = [
    { name: '징벌', path: '/board/징벌', desc: '징벌 플레이/루트/타이밍 토론' },
    { name: '일반', path: '/board/일반', desc: '일반 차사 운영/생존/상대법' },
    { name: '논란', path: '/board/논란', desc: '이슈 정리(팩트 기반)' },
    { name: '메타', path: '/board/메타', desc: '패치 후 메타/빌드 분석' },
  ]

  const anonId = useMemo(() => (typeof window !== 'undefined' ? getAnonId() : ''), [])
  const [nickname, setNickname] = useState('')
  const [loading, setLoading] = useState(false)
  const [summary, setSummary] = useState<string | null>(null)

  const analyze = async () => {
    const q = nickname.trim()
    if (!q) return

    setLoading(true)
    setSummary(null)

    // 1) posts에서 닉네임 언급 검색 (title/content)
    const { data: posts, error: pErr } = await supabase
      .from('posts')
      .select('id,board,title,content,author_anon_id,created_at')
      .eq('hidden', false)
      .or(`title.ilike.%${q}%,content.ilike.%${q}%`)
      .order('created_at', { ascending: false })
      .limit(50)

    if (pErr) {
      setSummary(`[Supabase] ${pErr.message}`)
      setLoading(false)
      return
    }

    const matchedPosts = (posts ?? []) as PostRow[]

    // 2) 해당 post들의 댓글에서 닉네임 언급 검색
    let matchedComments: CommentRow[] = []
    if (matchedPosts.length > 0) {
      const postIds = matchedPosts.map(p => p.id)
      const { data: cs, error: cErr } = await supabase
        .from('comments')
        .select('id,post_id,content,author_anon_id,created_at')
        .eq('hidden', false)
        .in('post_id', postIds)
        .ilike('content', `%${q}%`)
        .order('created_at', { ascending: false })
        .limit(80)

      if (cErr) {
        setSummary(`[Supabase] ${cErr.message}`)
        setLoading(false)
        return
      }

      matchedComments = (cs ?? []) as CommentRow[]
    }

    const mentionCount = matchedPosts.length + matchedComments.length

    // 데이터 부족 처리
    if (mentionCount < 3) {
      setSummary(
        `데이터가 부족해서 상세 분석이 어렵습니다.\n` +
        `현재 수집된 언급: ${mentionCount}개\n\n` +
        `게시글/댓글이 더 쌓이면 더 정확해집니다.`
      )
      setLoading(false)
      return
    }

    // 키워드 기반 “연구소 톤” 경향 (안전)
    const pool = [
      ...matchedPosts.map(p => `${p.title}\n${p.content}`),
      ...matchedComments.map(c => c.content),
    ].join('\n')

    const tags: string[] = []
    if (pool.includes('안정')) tags.push('안정형')
    if (pool.includes('무리') || pool.includes('돌격')) tags.push('공격성')
    if (pool.includes('팀') || pool.includes('커버') || pool.includes('서폿')) tags.push('팀플')
    if (pool.includes('후반')) tags.push('후반 운영')
    if (pool.includes('징') || pool.includes('징벌')) tags.push('징벌 관련')
    if (pool.includes('판단')) tags.push('판단/콜')

    const uniqTags = Array.from(new Set(tags))
    const tagLine = uniqTags.length ? uniqTags.join(' / ') : '(키워드 부족)'

    // 게시판 분포
    const byBoard = new Map<string, number>()
    matchedPosts.forEach(p => byBoard.set(p.board, (byBoard.get(p.board) ?? 0) + 1))
    const boardLine =
      Array.from(byBoard.entries())
        .sort((a, b) => b[1] - a[1])
        .map(([b, n]) => `${b} ${n}`)
        .join(', ') || '없음'

    setSummary(
      `커뮤니티 언급 ${mentionCount}개 기준 자동 요약입니다.\n\n` +
      `- 키워드 경향: ${tagLine}\n` +
      `- 게시판 분포(글 기준): ${boardLine}\n\n` +
      `※ 본 결과는 개인 평가가 아니라, 게시글/댓글 의견의 구조적 요약입니다.\n` +
      `※ 데이터가 더 쌓일수록 정확도가 올라갑니다.`
    )

    setLoading(false)
  }

  return (
    <main className="min-h-screen bg-zinc-950 text-white px-5 py-10">
      <div className="max-w-3xl mx-auto space-y-8">
        <header className="space-y-3">
          <h1 className="text-3xl font-bold">테일즈런너 차사잡아라 AI 익명 게시판</h1>
          <p className="text-zinc-300 whitespace-pre-line">{intro}</p>
          <p className="text-xs text-zinc-500">
            ※ 분석/정리는 개인 평가가 아닌, 게시판 의견의 구조적 요약입니다.
          </p>
        </header>

        <section className="bg-zinc-900 rounded-xl p-4 space-y-3">
          <div className="text-sm text-zinc-300">
            내 익명 ID: <span className="text-zinc-100">{anonId}</span>
          </div>

          <div className="flex gap-2">
            <input
              className="flex-1 rounded-lg px-3 py-2 text-black"
              placeholder="닉네임 입력 (예: TimeLeap)"
              value={nickname}
              onChange={(e) => setNickname(e.target.value)}
            />
            <button
              type="button"
              onClick={analyze}
              disabled={loading}
              className="rounded-lg px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-60"
            >
              {loading ? '분석중…' : '이 사람 어때?'}
            </button>
          </div>

          {summary && (
            <div className="bg-zinc-950 rounded-lg p-3 text-zinc-200 whitespace-pre-line">
              {summary}
            </div>
          )}
        </section>

        <section className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {boards.map((b) => (
            <Link
              key={b.path}
              href={b.path}
              className="rounded-xl bg-zinc-900 p-4 hover:bg-zinc-800 transition"
            >
              <div className="text-lg font-semibold">{b.name} 게시판</div>
              <div className="text-sm text-zinc-300 mt-1">{b.desc}</div>
            </Link>
          ))}
        </section>

        <footer className="text-xs text-zinc-500">
          사이트 주소: chasjabara.netlify.app (도메인은 나중에 연결 가능)
        </footer>
      </div>
    </main>
  )
}
